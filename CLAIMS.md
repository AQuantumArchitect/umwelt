...
| **Wall-clock pacing defeats sparse-cadence starvation**: a world that declares its natural tick (`spec.tick_s`) gets each gap honored as a bounded zero-order hold of the previous batch's inputs — daily batches land within 0.15 z of the dense per-hour stream, a hold spans ONE gap only (stale readings ease back to uncertainty), and tick_s=None provably never fires the path (the origin behavior, untouched). Forced by the first foreign-cadence world (daily market bars, which needed a hand-built republish burst). | `tests/test_wall_pacing.py` | PINNED |

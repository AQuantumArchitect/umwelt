...
class DomainSpec:
    ...
    # MEMBRANE CADENCE, not time. None (default) = ingest-driven: one compute step
    # per batch regardless of wall gaps — the origin's dense-polled behavior. Set it
    # and each delivered batch's inputs HOLD for the gap they close, advanced in
    # bounded unit compute steps (one per ingest_hold_s of gap) before the next
    # batch lands — a zero-order-hold adapter for sparse feeds, living entirely at
    # the ingest membrane. It models NOTHING about time: the world's clocks are
    # drivers (in-universe qubits, declared only if the domain cares), and dt is
    # compute step size. See docs/TIME.md for the three-times taxonomy.
    ingest_hold_s: float | None = None
    ...
    forecast_interaction_order: int = 2       # monomial order of the higher-order context (2 = pairwise products)
